...
2. Document the current seams and contract points in the generator pipeline: symbol extraction, pattern normalization, regex parsing, char-class optimization, and final text emission.
3. Define invariants that must remain unchanged: token ordering, skip behavior, diagnostics, namespace/type names, and deterministic output.

2. Phase 2 - Input IR introduction (depends on Phase 1)
...
2. Split current input processing into two responsibilities: Roslyn adapter (symbol/attribute access only) and IR normalizer/validator (token and regex semantics only).
3. Preserve existing regex model reuse by embedding or referencing parsed regex structures in the input IR rather than duplicating parser logic.
4. Add serialization/debug views for input IR to aid diagnostics and golden testing.

3. Phase 3 - Output IR introduction (depends on Phase 2)
...
2. Refactor emission routines to produce output IR nodes first, then render through a dedicated C# renderer.
3. Keep a compatibility fallback node kind for raw statement/text fragments to reduce migration risk for complex constructs.
4. Ensure rendering remains deterministic through stable ordering rules and explicit formatting policies.

4. Phase 4 - SExpr versus typed IR decision gates (parallel analysis with Phase 2 design details)
1. Evaluate three architecture options for both layers using prototypes and objective criteria.
2. Option A: SExpr for both input and output IR.
3. Option B: typed input IR plus typed output IR.
4. Option C: hybrid, typed input IR plus SExpr output IR (or inverse only if strongly justified).
5. Make independent decisions per layer because optimal choice may differ between input and output concerns.

5. Phase 5 - Incremental migration and risk control (depends on Phases 3 and 4)
1. Migrate one emitter slice first (for example, literal token matching) to validate IR shape and rendering strategy.
2. Migrate regex-based emitters and char-class emission next; keep parity checks at each step.
3. Remove direct string-writing paths only after parity is proven for all generator outputs.
4. Retain temporary dual-path comparison mode during rollout to detect regressions early.

6. Phase 6 - Testing and verification hardening (parallelizable after Phase 3 starts)
1. Add input-IR unit tests for attribute mapping, validation rules, and normalized model shape.
2. Add output-IR tests for control-flow structure and renderer correctness.
3. Add end-to-end snapshot tests comparing generated C# against approved baselines.
4. Add deterministic-output checks across repeated runs and different machine environments.

7. Phase 7 - Documentation and adoption (depends on Phase 5)
1. Document the new pipeline and extension points for future token pattern features.
2. Document IR schema/conventions, especially if SExpr is chosen (node naming, required fields, canonical ordering).
3. Add contributor guidance on where to place new logic: Roslyn adapter vs input normalization vs output transformation vs renderer.
